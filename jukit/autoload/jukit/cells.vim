...
fun! s:new_marker(id1, id2, lnum, markdown) abort
    ...
    if g:jukit_use_tcomment == 1
        if a:markdown
          call append(lnum, ' %% [markdown] <' . id1 . '|' . id2 . '>')
        else
        ...
        echom a:markdown
        if a:markdown
          call append(lnum, g:jukit_comment_mark . ' %% [markdown] <' . id1 . '|' . id2 . '>')
        else
          call append(lnum, g:jukit_comment_mark . ' %% <' . id1 . '|' . id2 . '>')
        endif
    endif
...
fun! jukit#cells#create_below(markdown) abort
    ...
    if markers['below']['pos']
        ...
        call append(pos-1, ['', '', ''])
        call s:new_marker(markers['below']['ids'][0], cell_id, pos-1, a:markdown)
        call cursor(pos+2, 1)
    elseif markers['above']['pos']
        call append(line('$'), ['', '', ''])
        call s:new_marker(markers['above']['ids'][1], cell_id, line('$')-3, a:markdown)
        call cursor(line('$')-1, 1)
    else
        call append(line('$'), ['', '', ''])
        let ids = s:new_marker(-1, cell_id, line('$')-3, a:markdown)
...
fun! jukit#cells#create_above(markdown) abort
    ...
    if markers['above']['pos'] != 0
        ...
        call append(pos-1, ['', '', ''])
        call s:new_marker(markers['above']['ids'][0], cell_id, pos-1, a:markdown)
        call cursor(pos+2, 1)
    elseif markers['below']['pos'] != 0
        call append(0, ['', '', ''])
        call s:new_marker(cell_id, markers['below']['ids'][0], 3, a:markdown)
        call cursor(2, 1)
    else
        call append(0, ['', '', ''])
        let ids = s:new_marker(cell_id, -1, 3, a:markdown)
